...
public:
	...
	Binary_Tree() : root(NULL) { num_items = 0; }
	...
	*/
	...
	Item_Type* convert_vector(){
		Item_Type* arr = new Item_Type[num_items];
		to_vector(arr, root, 0);
		return arr;
	}

	void to_vector(Item_Type*& array,BTNode<Item_Type>* root,int index){
		array[index] = root->data;
		if (root->left != NULL)
			to_vector(array, root->left, 2 * index + 1);
		if (root->right!=NULL)
			to_vector(array, root->right, 2 * index + 2);
	...
	bool is_complete_tree(){
		...
		int max_height = -1;
		int latest_height = -1;
		is_complete_tree(root, 0, result, max_height, latest_height);

		return result;
	}

	void is_complete_tree(BTNode<Item_Type>* local_root,int height, bool& result, int& max_height, int& latest_height){
		...
		if (local_root == NULL){
			...
			if (height > latest_height || height < max_height - 1)
				result = false;
			latest_height = height;
		}
		else{
			is_complete_tree(local_root->left, height + 1, result, max_height, latest_height);
			is_complete_tree(local_root->right, height + 1, result, max_height, latest_height);

		}
	}


	BTNode<Item_Type>* is_binary_search(BTNode<Item_Type>* local_root,bool& result){

		BTNode<Item_Type>* last_node = local_root;

		if (result == false) //just terminate
			return local_root;
		
		if (local_root != NULL) {
			if (local_root->left != NULL) {
				last_node=is_binary_search(local_root->left,result);
				
				if (local_root->data < last_node->data){
					result = false;
					return local_root;
				}
			}
			...
			if (local_root->right != NULL) {
				last_node = is_binary_search(local_root->right,result);
				if (local_root->data > last_node->data){
					result = false;
					return local_root;
				}
...
protected:
	...
	Binary_Tree(BTNode<Item_Type>* new_root) : root(new_root) { num_items = 0; }
